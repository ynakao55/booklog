class BooksController < ApplicationController
  ...
  def index
    ...
    @books = if @status.present? && Book.statuses.key?(@status)
             Book.where(status: Book.statuses[@status]).order(created_at: :desc)
           else
  ...
  private
    ...
    def book_params
      params.expect(book: [ :title, :author, :status, :note ])
    end
end
